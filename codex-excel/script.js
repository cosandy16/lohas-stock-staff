...
function renderChart(analysis) {
  ...
  chart.innerHTML = `
    <svg viewBox="0 0 ${width} ${height}" role="img" aria-label="股市樂活五線譜">
      ...
      <text x="${x(analysis.length - 1) - 10}" y="${y(last.close) - 12}" text-anchor="end" font-size="14" font-weight="900" fill="#17202f">${formatPrice(last.close)}</text>
...
function render() {
  try {
    ...
    zoneText.textContent = priceZone(latest);
